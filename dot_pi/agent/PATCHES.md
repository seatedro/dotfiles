...
Patch the bundled Pi TUI autocomplete implementation inside the global Pi install:
...
In `CombinedAutocompleteProvider.getSuggestions(...)`, after `@` file autocomplete handling and before `/` command autocomplete handling, add a `$` prefix branch that:
...
If patched, this should print lines mentioning `extractSkillPrefix` and `skill invocation`.
